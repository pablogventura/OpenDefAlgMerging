...
def TupleModelHashEstrella(model, generator_tuple):
    """
    Hace HIT para todas las permutaciones a la vez
    """


    generator_tuple = list(generator_tuple)
    model = model
    V = list(generator_tuple)

    b_i=0
    b_n=len(generator_tuple)
    sigmas = list(perm)
    perms = list(perm)
    ...
    H = [generator_tuple]
    i = len(generator_tuple)-1
    T = defaultdict(set, {a: {j} for j, a in enumerate(generator_tuple)})
    O = H[-1]
        ...
        for ar in sorted(ops):
            for f in sorted(ops[ar], key=lambda f: f.sym):
                ...
                for perm in permutaciones:
                    ...
                    sigma += [H[-1].index(e)+len(sigma) for e in H[-1]]
        O = H[-1]
        # aca se ha calculado un bloque de hit
        
    T = {k: frozenset(T[k]) for k in self.T}
    H.pop(-1)
